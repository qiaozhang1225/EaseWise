...
def build_fortune_prompts(payload: dict[str, Any], *, tone_pack: TonePack = "customer") -> tuple[str, str, dict[str, Any]]:
    locked = _locked_fields(payload)
    evidence = _evidence_fields(payload)
    shared_foundation = load_shared_foundation()
    model_pack = load_aspect_model_pack("fortune", tone_pack)
    aspect_knowledge = load_aspect_knowledge("fortune")
    taxonomy = load_aspect_taxonomy("fortune")
    output_contract = load_aspect_output_contract("fortune")
    style_examples = load_aspect_style_examples("fortune")

    json_example = {
        "core_judgement": f"这个号码的运势状态属于{locked['level']}，整体更接近{locked['type']}，主因落在{locked['primary_driver']}。",
        "real_world_manifestation": locked["manifestation"],
        "user_facing_paragraph": "请在这里输出一段完整、自然、专业的中文运势说明。不要套固定起手，要围绕这个号码当前整体势能最突出的矛盾去组织语言。",
    }

    system_prompt = (
        "你是易如反掌的运势解释渲染器，也是一个长期给人看手机号当前阶段起势、回拉与顺逆感的老师。\n"
        "你的职责不是改分，而是基于代码锁定事实和统一知识底座，写出自然、专业、但又让用户能听明白的运势说明。\n"
        "必须遵守这些硬规则：\n"
        "1. 不得修改或推翻 locked_fields。\n"
        "2. 不得新造运势档位、类型、分数或建议。\n"
        "3. 如遇 mixed signals，只解释主矛盾，不改 code-locked 结论。\n"
        "4. 不要把空亡、门迫、入墓、击刑逐条抄成 checklist。\n"
        "5. 不要主动发明 evidence 里没有的特殊格局，也不要把合十格带进运势结论。\n"
        "6. 最终只输出一个 JSON object，不输出 Markdown，不输出额外说明。\n"
        "7. 最终段落里必须明确回答：如果用户看重当前阶段的运势、顺势推进和少折腾，这个号能不能继续长期使用。\n"
        "8. 语气要像真实分析者在解释整体气口、承接和回拉，不像数据库字段拼接。\n"
        "9. user_facing_paragraph 不必第一句就重复档位，可以从主矛盾、关键阶段或现实使用感起笔。"
    )

    user_prompt = (
        "请基于以下统一知识底座、运势判断知识、风格样例、输出合同、locked_fields 和 evidence，输出一个 JSON object。\n\n"
        "【统一知识底座】\n"
        f"{shared_foundation}\n\n"
        "【当前语气包】\n"
        f"{model_pack}\n\n"
...
def render_fortune_from_package(
    ...
    *,
    ...
    payload = package["score_template"]["fortune_payload"]
    client = client or DeepSeekClient.from_env()
    system_prompt, user_prompt, json_example = build_fortune_prompts(payload, tone_pack=tone_pack)
    ...
    return FortuneRenderResult(
        fortune_level=payload["level"],
        fortune_type=payload["type"],
...
def render_fortune_from_phone(
    ...
    *,
    ...
    return render_fortune_from_package(
        package,
        tone_pack=tone_pack,
        client=client,
        model=model,
...
def _fallback_core_judgement(payload: dict[str, Any], *, tone_pack: TonePack) -> str:
    if tone_pack == "professional":
        return f"从整体盘势看，这个号码的运势状态落在{payload['level']}，主轴更接近{payload['type']}，主因落在{payload['primary_driver']}。"
    return f"这个号码当前的运势状态属于{payload['level']}，整体更接近{payload['type']}，主因落在{payload['primary_driver']}。"
...
def _fallback_paragraph(payload: dict[str, Any], *, tone_pack: TonePack) -> str:
    ...
    if tone_pack == "professional":
        paragraph = (
            f"这个号码放在运势层面，当前落在{payload['level']}，主轴更接近{payload['type']}。{payload['manifestation']}"
            f"{extra}当前更值得盯住的是{watch_text}，因为它决定了这个号是顺着气口往上走，还是在关键节点被回拉、被拖慢。"
        )
    else:
        paragraph = (
            f"这个号码看运势，当前属于{payload['level']}，主轴更接近{payload['type']}，关键不只是表面有没有起色，而是起了势以后能不能顺着走下去。{payload['manifestation']}"
            f"{extra}它更值得留意的是{watch_text}，这些地方往往就是现实里最容易把顺势感拉回来的位置。"
        )
...
def _fallback_gap_note(payload: dict[str, Any]) -> str:
    ...
    if score_gap <= 0:
        return "这类盘面的起势感、承接感和持续性相对完整。"
    if score_gap <= 12:
        return "它不是没有势，而是边角位置还留着一点回拉。"
    if score_gap <= 25:
        return "它的问题不在完全无势，而在势能起落时更吃承接和节奏。"
    return "它的主矛盾不在表面热闹，而在关键节点容易被回拉和消耗。"
