...
def build_wealth_prompts(payload: dict[str, Any], *, tone_pack: TonePack = "customer") -> tuple[str, str, dict[str, Any]]:
    locked = _locked_fields(payload)
    evidence = _evidence_fields(payload)
    shared_foundation = load_shared_foundation()
    model_pack = load_aspect_model_pack("wealth", tone_pack)
    aspect_knowledge = load_aspect_knowledge("wealth")
    taxonomy = load_aspect_taxonomy("wealth")
    output_contract = load_aspect_output_contract("wealth")
    style_examples = load_aspect_style_examples("wealth")

    json_example = {
        "core_judgement": f"这个号码的财运状态属于{locked['level']}，整体更接近{locked['type']}，主因落在{locked['primary_driver']}。",
        "real_world_manifestation": locked["manifestation"],
        "user_facing_paragraph": "请在这里输出一段完整、自然、专业的中文财运说明。不要固定套话，要围绕这个号码在财路里最突出的结构矛盾去组织语言。",
    }

    system_prompt = (
        "你是易如反掌的财运解释渲染器，也是一位长期给人看手机号财路盘面的老师。\n"
        "你的职责不是改分，而是基于代码锁定事实和统一知识底座，写出自然、专业、但用户能听懂的财运说明。\n"
        "必须遵守这些硬规则：\n"
        "1. 不得修改或推翻 locked_fields。\n"
        "2. 不得新造财运档位、类型、分数或建议。\n"
        "3. mixed signals 只解释主矛盾，不改 code-locked 结论。\n"
        "4. 不要把空亡、门迫、入墓、击刑逐条抄成 checklist。\n"
        "5. 不要主动发明 evidence 里没有的特殊格局，也不要把合十格带进财运结论。\n"
        "6. 最终只输出一个 JSON object，不输出 Markdown，不输出额外说明。\n"
        "7. 段落里必须明确回答：如果用户看重财运，这个号能不能继续长期使用。\n"
        "8. 语气要像真实分析者在解释这条财路的结构，不像数据库字段拼接。"
    )

    user_prompt = (
        "请基于以下统一知识底座、财运判断知识、风格样例、输出合同、locked_fields 和 evidence，输出一个 JSON object。\n\n"
        "【统一知识底座】\n"
        f"{shared_foundation}\n\n"
        "【当前语气包】\n"
        f"{model_pack}\n\n"
        ...
        "输出要求：\n"
        "- core_judgement：1 句话，点明财运结论、类型和主因。\n"
        "- real_world_manifestation：1 句话，贴近 locked_fields.manifestation，不跑题。\n"
        "- user_facing_paragraph：一段完整中文财运说明，要有厚度，但不要写成固定模板。\n"
        "- 可以自由选择切入角度：财路主轴、主扣分、现实场景、长期建议都可以。\n"
        "- 不要把所有号码都写成一样的开头。\n"
        "- 不要把四害写成 checklist。\n"
...
def render_wealth_from_package(
    ...
    *,
    ...
    payload = package["score_template"]["wealth_payload"]
    client = client or DeepSeekClient.from_env()
    system_prompt, user_prompt, json_example = build_wealth_prompts(payload, tone_pack=tone_pack)
    ...
    return WealthRenderResult(
        wealth_level=payload["level"],
        wealth_type=payload["type"],
...
def render_wealth_from_phone(
    ...
    *,
    ...
    return render_wealth_from_package(
        package,
        tone_pack=tone_pack,
        client=client,
        model=model,
...
def _fallback_core_judgement(payload: dict[str, Any], *, tone_pack: TonePack) -> str:
    if tone_pack == "professional":
        return f"从财路承接力看，这个号码归在{payload['level']}，主轴更接近{payload['type']}，主要矛盾落在{payload['primary_driver']}。"
    return f"这个号码的财运状态属于{payload['level']}，整体更接近{payload['type']}，主因落在{payload['primary_driver']}。"
...
def _fallback_paragraph(payload: dict[str, Any], *, tone_pack: TonePack) -> str:
    ...
    if tone_pack == "professional":
        base = (
            f"从财路承接力看，这个号码归在{payload['level']}，主轴更接近{payload['type']}。"
            f"真正需要看的，是{payload['primary_driver']}与{payload['secondary_driver']}之间，能不能把进财和留财稳稳接住。"
    ...
    base = (
        f"这个号码放在财运里，主轴落在{payload['level']}这一档，更接近{payload['type']}。"
        f"它更怕的不是有没有来钱，而是{payload['primary_driver']}能不能把这条财路长期接住。"
        f"现实里更容易表现成{payload['manifestation']}"
...
def _fallback_gap_note(payload: dict[str, Any]) -> str:
    ...
    reason_text = "；".join(reasons[:2]) if reasons else "还有一些财路结构上的扣分"
    area_text = "、".join(areas[:2]) if areas else "关键财务场景"
...
def _paragraph_reflects_usage_advice(paragraph: str, advice: str) -> bool:
    ...
    positive_markers = ("长期使用", "继续长期使用", "继续使用", "坚持使用", "财运主用")
    negative_markers = ("不建议继续长期使用", "不建议当财运主用", "建议优先调整", "不建议继续用")

    if "坚持使用" in advice_text or "财运主用" in advice_text:
        return any(marker in paragraph for marker in ("坚持使用", "长期使用", "继续长期使用", "财运主用"))
    if any(marker in advice_text for marker in ("不建议继续长期使用", "不建议当财运主用", "优先调整")):
