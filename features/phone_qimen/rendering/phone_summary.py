...
def build_phone_summary_prompts(payload: dict[str, Any], *, tone_pack: TonePack = "customer") -> tuple[str, str, dict[str, Any]]:
    ...
    json_example = {
        "title": "这组号码能把事情推起来，但最怕后面收不稳",
        "risk": "这个号的亮点不是没有，而是容易在执行后半段被关系、流程或突发状况拖住。用户现实里更容易感到前面有机会、有人找、事情也能启动，但一到收口、合作分工或结果兑现时，就会反复多、消耗大、差一口气。",
        "usage_guidance": (
            "如果继续使用，更适合把它放在开拓、联系、推进事情的位置，而不是独自承担所有收尾和高压力合作。"
    ...
    system_prompt = (
        ...
        "5. title、risk、usage_guidance 是用户主段落，必须以用户能对照的状态、场景和使用建议为主，不得堆术语。\n"
        "6. risk 必须是一段话，控制在两到三句，讲清楚核心问题怎么落到现实里，例如推进、合作、回款、关系、情绪或收尾。\n"
        "7. usage_guidance 必须是一段话，要像一个有经验的人在直接判断号码，不要拆条，不要列表。\n"
