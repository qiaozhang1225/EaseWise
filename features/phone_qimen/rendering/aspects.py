...
def build_aspect_prompts(
    ...
    *,
    ...
    system_prompt = (
        ...
        "12. 专业词不是禁用词，但必须先点名、再翻译、再落到现实场景；例如写“双关”时，要解释为同一组信息同时牵动两个主题，现实里容易一件事连带影响关系或钱。\n"
        "13. title、content、risk 不要出现“结构承接、可用空间、可用之处、现实承接感、底层结构、后段承接、结构封顶”等内部分析词。\n\n"
        f"{shared_foundation}\n\n"
