...
- 状态：`planned / blocked by external setup`
- 当前阶段只做产品卡片和 PRD 口径，不接入真实阿里云短信、微信 H5 登录或小程序登录
- 目标：
  - 以手机号作为正式主账号，保证用户未来在 App 中可通过手机号登录并继承资产
  - 以 UnionID 作为微信体系内跨 H5、公众号、小程序和多小程序的统一识别字段
  - Session 仅作为临时访问态和登录态承载，不作为长期用户身份
- 未来需要接入：
  - 阿里云手机号短信验证
  - 手机号 + 密码登录
  - 微信 H5 获取 UnionID
...
- 状态：`planned / blocked by auth setup`
- 当前阶段不实现登录弹窗、短信验证或 UnionID 登录领取流程
- 规则目标：
  - 免费初始积分不再按 Session 发放
  - 未登录用户只可查看当前页面状态
  - 未登录用户进入关键流程时，前台提示 `登录以获取 XX 积分`
  - 用户完成手机号验证或 UnionID 登录后，才可领取一次免费初始积分
  - 同一手机号或同一 UnionID 只能领取一次
- 过渡说明：
  - 当前代码中仍存在游客 Session 与游客初始积分配置
  - 等手机号短信验证或微信 UnionID 登录具备上线条件后，再执行游客积分停发切换
