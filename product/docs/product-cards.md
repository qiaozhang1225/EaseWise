...
## User And Auth Backlog

### USER-001 用户身份体系升级储备

- 状态：`in progress / phone password login implemented`
- 当前阶段已落地手机号 + 密码注册登录，真实阿里云短信、微信 H5 登录或小程序登录仍作为后续接入项
- 目标：
  - 以手机号作为正式主账号，保证用户未来在 App 中可通过手机号登录并继承资产
  - 以 UnionID 作为微信体系内跨 H5、公众号、小程序和多小程序的统一识别字段
  - 停用临时浏览身份体系，不再自动创建临时用户或发放积分
- 未来需要接入：
  - 阿里云手机号短信验证
  - 微信 H5 获取 UnionID
  - 微信小程序获取 UnionID
  - 多小程序 AppID 配置与统一 UnionID 资产打通
- 识别规则：
  - 业务主身份优先看手机号和 UnionID
  - OpenID 只作为微信单应用内的技术排查字段保留
  - 同一 UnionID 下的多个小程序访问，应命中同一个系统用户资产
- 资产连续性要求：
  - 积分
  - 可提现余额
  - 冻结中返佣余额
...
- 状态：`in progress / formal login only`
- 当前阶段已实现手机号 + 密码注册登录；短信验证与 UnionID 登录领取流程后续接入
- 规则目标：
  - 免费初始积分只发给正式注册用户
  - 未登录用户只可查看当前页面状态
  - 未登录用户进入关键流程时，前台提示登录或注册
  - 用户完成手机号注册、手机号验证或 UnionID 登录后，才可领取一次免费初始积分
  - 同一手机号或同一 UnionID 只能领取一次
- 过渡说明：
  - 当前版本不再创建临时用户，也不再保留临时初始积分配置
  - 手机号短信验证与微信 UnionID 登录具备上线条件后，只补充正式身份绑定能力，不恢复临时身份积分逻辑

### USER-003 停用临时身份与用户状态收束

- 状态：`implemented locally / pending verification`
- 当前执行范围：
  - 停用自动创建临时用户的接口和前端启动逻辑
  - 手机号、微信等正式登录继续使用 bearer token
  - 本地开发库清理用户域测试数据，保留系统配置和第三方服务密钥配置
  - 用户状态统一收束为 `active / disabled`
- 前端要求：
  - 未登录只允许浏览安全内容
  - 评测、解锁、充值、智能体等资产或记录相关动作必须先登录
  - 个人中心未登录态显示 0 积分，并引导登录 / 注册
- 后台要求：
  - 用户状态筛选和编辑仅保留 `正常 / 禁用`
  - 旧状态只作为迁移兼容，不作为后台常规选项展示

## Promotion Backlog

### PROMO-001 推广资产模型修正

- 状态：`planned / ready for local implementation`
- 系统级口径：
  - 推广收益不再建立第二套积分账户
  - 系统内只保留一种 `积分`，用于功能消费
  - 推广返佣改为资金余额模型
...
- 规则调整：
  - 返佣不再支持转换为积分消费
  - 用户积分不足时，不再使用推广收益抵扣功能消费
  - 提现申请提交后冻结对应可提现余额，避免重复提现
  - 订单退款或返佣撤回时，优先冲减冻结中返佣余额；已结算的从可提现余额中冲减
- 后台调整方向：
  - 用户管理展示积分、可提现余额、冻结中返佣余额
  - 推广合作展示返佣金额、冻结余额、提现金额和资金状态
  - 后续代码层再将现有推广收益相关字段迁移为资金余额字段
...
- 位置：`系统配置 > 第三方服务密钥管理`
- V1 目标：
  - 支持在后台配置 DeepSeek 与阿里云服务密钥
  - 运行时优先读取后台启用配置，未配置时继续读取服务器环境变量
  - 不在列表、详情或编辑表单中回显真实密钥明文
- 页面能力：
  - 支持查看密钥名称、供应商、模型或服务、脱敏值、真实 Key 配置状态、启用状态、优先级、备注、最后更新时间、最后操作人
  - 支持新增、编辑、启用/停用、删除服务密钥
  - 新增时必须填写真实 Key，编辑时真实 Key 留空则保留原密钥
- 后端能力：
  - 使用 `llm_api_keys` 数据表或等价配置模型保存服务密钥配置
  - 服务端生成 `masked_key` 与 `secret_ref`，前端不手动维护脱敏值
  - DeepSeek 调用链优先读取 `provider=deepseek` 的启用密钥
  - 语音云端优先级为阿里云 NLS TTS、阿里云百炼 CosyVoice、浏览器兜底
  - NLS TTS 配置完整时优先使用 `tts_app_key`、`nls_access_key_id`、`nls_access_key_secret`
  - 阿里云百炼 TTS 读取 `provider=aliyun` 且服务名为 `bailian_api_key` 的启用密钥，作为 NLS 不可用时的第二优先级
  - 为兼容早期后台配置，如果 `provider=aliyun`、服务名为 `tts_app_key` 的密钥值是 `sk-` 开头，也按百炼 API Key 使用
  - 旧 NLS TTS 使用 `tts_app_key`、`nls_access_key_id`、`nls_access_key_secret`，由后端通过阿里云 SDK 自动获取并缓存 Token
  - `tts_token` 仅作为手动兜底配置，不作为推荐维护方式
- 后续扩展：
  - 并发监控
  - 失败率统计
  - 自动切换 key
