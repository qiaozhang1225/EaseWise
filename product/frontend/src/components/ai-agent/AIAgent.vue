    ...
    <transition name="fade" mode="out-in">
      ...
      >
        ...
        <div class="space-y-2 max-w-[90%]">
          <h2 class="font-serif text-[20px] font-bold text-brand-ink-strong">易如反掌 · 智能体决策助手</h2>
          <p class="text-[13px] text-brand-secondary leading-relaxed">
            为提供定制的奇门排盘和黄历沟通服务，智能体需要关联您的手机测算历史与资产。游客无法免费启用连续对话功能。
          </p>
        </div>

        <div class="bg-white p-4 rounded-xl border border-gray-100 max-w-[95%] text-left space-y-2 flex items-start gap-2 text-[12px] text-brand-secondary">
        ...
        <button 
          @click="handleLogin"
          class="w-full py-3.5 bg-brand-primary text-white rounded-xl font-bold text-[14px] shadow-sm hover:bg-brand-primary-strong active:scale-[0.98] transition-all cursor-pointer outline-none flex items-center justify-center gap-2"
        >
          <Smartphone :size="16" />
      ...
      >
        ...
        <div class="px-margin-mobile pt-3 shrink-0 flex items-center justify-between gap-2">
          <div class="flex-1 bg-white p-3 rounded-xl border border-gray-100/80 flex items-start gap-2.5 shadow-sm">
            ...
            <p class="text-[11.5px] text-brand-secondary leading-relaxed font-semibold">
        ...
        <div ref="chatScrollContainerRef" class="flex-1 min-h-0 overflow-y-auto overscroll-contain px-margin-mobile py-4 pb-4 flex flex-col gap-4.5 no-scrollbar">
          ...
          >
            ...
            <div class="flex flex-col gap-1 w-full text-left">
              <!-- Message bubble -->
              <div class="px-3.5 py-2.5 rounded-2xl border text-[13.5px] leading-relaxed shadow-sm"
              ...
              <div class="flex items-center gap-2 px-1 text-[9.5px] font-mono text-brand-secondary/60"
              ...
              <div v-if="msg.role === 'assistant' && msg.chips" class="flex flex-wrap gap-1.5 mt-2">
                <button
                  ...
                  class="bg-brand-paper hover:bg-gray-100 text-brand-primary border border-gray-200/50 px-2.5 py-1 rounded-full text-[10.5px] font-bold transition-all outline-none shrink-0"
                >
                  {{ chip }}
                </button>
              </div>
        ...
        <div class="bg-brand-paper/95 backdrop-blur-sm border-t border-gray-100/80 shrink-0">
          ...
          <div class="px-margin-mobile py-2.5 overflow-x-auto no-scrollbar flex gap-2 w-full">
            <button 
              ...
              :key="q"
              @click="handleSend(q)"
              class="whitespace-nowrap px-3 py-2 bg-white border border-gray-200 rounded-full text-[11px] text-brand-secondary font-bold hover:border-brand-primary transition-colors cursor-pointer outline-none shadow-sm"
            >
              {{ q }}
            </button>
          </div>
          ...
          <div class="px-4 pb-4 flex items-center gap-3">
            <div class="flex-1 bg-white p-1 rounded-2xl flex items-center gap-1.5 border border-gray-100 shadow-sm pr-3">
              <input 
                class="bg-transparent border-none focus:ring-0 w-full pl-3 pr-1 py-2 text-[14px] text-brand-ink-strong outline-none placeholder-gray-400 font-medium"
