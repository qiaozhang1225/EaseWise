# EaseWise H5 Source Bundle For New AI Studio Project

## Usage

Paste this bundle after `Prompt.md` in the new AI Studio project. Recreate every file at the exact path shown. This is an H5/mobile prototype bundle only; it intentionally excludes the backend admin management system and the large AdminWorkspace appendix. The bundled `src/App.vue` is adapted for AI Studio H5 scope by removing the `/admin` fallback while preserving the user-facing routes and contracts.

## Scope

- Included: H5 home, phone analysis, Four Pillars, Meihua prototype, AI Agent, profile, recharge, points claim, auth/support overlays, shared API/state/types/CSS.
- Excluded: backend admin management system, `AdminWorkspace.vue`, `AdminSelect.vue`, mock-only server authority, zip/binary/build artifacts.
