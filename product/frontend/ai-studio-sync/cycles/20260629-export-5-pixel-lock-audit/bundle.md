# EaseWise Pixel-Lock Source Bundle

## Purpose

This bundle contains the local source-of-truth files required to force pixel-level replication of the current EaseWise H5 pages that still drift in AI Studio: phone review, Four Pillars, profile, and AI Agent. Recreate each file exactly at the path shown. Do not summarize or redesign these files.

## Included Files

- `src/components/analysis/Analysis.vue` (114609 bytes)
- `src/components/four-pillars/FourPillarsAnalysis.vue` (156305 bytes)
- `src/components/four-pillars/FourPillarsNatalTable.vue` (29412 bytes)
- `src/components/profile/Profile.vue` (48272 bytes)
...
- `src/composables/useEaseWiseApp.ts` (37461 bytes)
- `src/lib/api.ts` (52869 bytes)
- `src/types/api.ts` (37803 bytes)
- `src/config/pricing.ts` (176 bytes)
- `src/constants/storage.ts` (669 bytes)
- `src/index.css` (2785 bytes)

Total source bytes: `549260`
