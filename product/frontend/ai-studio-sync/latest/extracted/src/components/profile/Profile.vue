...
const combinedHistory = computed(() => {
  ...
  const merged = [...phoneItems, ...baziItems];
...
function formatReviewStatus(review: any): string {
  ...
  return review.progress_message || '处理中';
}

function resolveReviewActionText(review: any): string {
...
async function handleOpenReview(review: any): Promise<void> {
  ...
  <div class="pt-4 pb-32 max-w-md mx-auto px-margin-mobile relative text-left">
    ...
    <transition name="fade">
      <div v-if="activeModal === 'history'" class="fixed inset-0 bg-brand-ink-strong/60 flex items-center justify-center p-4 z-[100] backdrop-blur-sm">
        <div class="bg-white rounded-2xl p-5 w-full max-w-md space-y-4 hairline-border shadow-2xl relative">
          <div class="flex justify-between items-center pb-2 border-b border-gray-100 w-full">
            <div class="text-left">
              ...
              <p class="font-sans text-[11px] text-brand-secondary">支持快捷查看您的数字奇门与四柱八字测算详情</p>
            </div>
          ...
          <div class="max-h-[380px] overflow-y-auto no-scrollbar space-y-3 py-1 pr-1">
            <p
            ...
            <div v-if="combinedHistory.length > 0" class="space-y-3">
              <button
                ...
                type="button"
                class="w-full bg-brand-paper/50 p-3.5 rounded-xl border flex items-center justify-between gap-3 text-xs text-left transition-all hover:bg-white disabled:opacity-60 disabled:cursor-not-allowed"
                :class="review.type === 'phone' ? 'border-sky-100/70 hover:border-sky-200' : 'border-amber-100 hover:border-amber-200'"
              ...
              >
                <div class="min-w-0 flex-1 space-y-1 text-left">
                  ...
                  <div class="flex items-center gap-1.5 flex-wrap">
                    <span 
                      class="font-sans text-[9px] font-bold px-1.5 py-0.5 rounded"
                      :class="review.type === 'phone' ? 'bg-sky-50 text-sky-600 border border-sky-100/50' : 'bg-amber-50 text-amber-700 border border-amber-100/50'"
                    >
                      {{ review.type === 'phone' ? '数字奇门' : '四柱八字' }}
                    </span>
                    <span class="font-sans text-[10px] text-brand-secondary/60">
                  ...
                  <div class="space-y-1 mt-1 text-left">
                    <p v-if="review.type === 'phone'" class="font-sans text-[13px] font-bold text-brand-ink-strong flex items-center gap-1">
                      <span class="text-brand-secondary/60 text-[11px] font-normal">评测号码:</span>
                      <span class="font-sans font-black tracking-wide text-brand-primary-strong">{{ review.masked_phone }}</span>
                      <span class="text-[10px] font-normal text-brand-secondary ml-1 bg-sky-50/50 border border-sky-100/20 px-1.5 py-0.5 rounded">
                        {{ review.gender === 'male' ? '男命' : '女命' }}
                      </span>
                    </p>
                    <div v-else-if="review.type === 'bazi'" class="space-y-1 text-left">
                      <p class="font-serif text-[13px] font-bold text-brand-ink-strong flex items-center gap-1">
                        <span class="text-brand-secondary/60 font-sans text-[11px] font-normal">评测命造:</span>
                        <span class="text-amber-900">{{ review.name?.trim() || '生辰盘主' }}</span>
                        <span class="font-sans text-[10px] font-normal text-brand-secondary/80 ml-1.5 bg-amber-50/50 border border-amber-100/30 px-1.5 py-0.5 rounded">
                          {{ review.gender === 'male' ? '乾造·男' : '坤造·女' }}
                        </span>
                      </p>
                      <p class="font-sans text-[10.5px] text-brand-secondary leading-snug flex items-center gap-1">
                        <span class="text-brand-secondary/50">生辰:</span>
                    ...
                    <p class="font-sans text-[10px] text-brand-secondary/80 leading-normal mt-0.5">
                ...
                <div class="shrink-0 flex flex-col items-end gap-1.5">
                  <span 
                    class="font-mono text-[11.5px] font-black px-2 py-0.5 rounded-full shrink-0"
                    :class="[
                      review.status === 'completed' 
                        ? (review.type === 'phone' ? 'bg-sky-50 text-sky-700 border border-sky-100/50' : 'bg-emerald-50 text-emerald-700 border border-emerald-100/40') 
                        : 'bg-zinc-50 text-zinc-500 border border-zinc-150'
                    ]"
                  >
                    {{ formatReviewStatus(review) }}
                  </span>
                  <ChevronRight :size="14" class="text-brand-secondary/40" />
                </div>
              </button>
            </div>
            <div v-else class="py-12 text-center space-y-2">
              <p class="font-sans text-[13px] text-brand-secondary">暂无评测历史记录。</p>
              <p class="font-sans text-[10.5px] text-brand-secondary/50 px-6 leading-relaxed">
                您可以先完成一次手机号评测，或者到“四柱八字”起盘排盘，您的排盘推演即会自动妥善保存。
              </p>
            </div>
          </div>

          <button @click="activeModal = null" class="w-full py-2.5 bg-brand-paper hover:bg-gray-100 text-brand-ink font-sans text-[13px] font-semibold rounded-lg outline-none cursor-pointer border-none">
            关闭窗口
          </button>
        </div>
      </div>
