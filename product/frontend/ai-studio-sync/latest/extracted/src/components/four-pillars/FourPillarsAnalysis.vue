...
onMounted(() => {
  ...
  if (!birthYear.value) birthYear.value = '1995';
  if (!birthMonth.value) birthMonth.value = '10';
  if (!birthDay.value) birthDay.value = '24';
  if (!birthTime.value) birthTime.value = '10:30';
  if (!profileName.value) profileName.value = '同修小易';
  if (!birthPlace.value) birthPlace.value = '北京市';
});
...
async function tryUnlockAspectWithWait(reviewId: string, aspectKey: string, title: string): Promise<void> {
  ...
  try {
    const refreshedReview = await unlockFourPillarsAspect(reviewId, aspectKey);
...
function persistCompletedReviewState(review: FourPillarsReviewRecord): void {
...
<template>
  <div class="min-h-screen bg-brand-paper pb-28 text-left">
    ...
    <header class="sticky top-0 z-20 bg-brand-paper/95 backdrop-blur border-b border-white/80">
      <div class="max-w-md mx-auto px-margin-mobile py-3 flex items-center justify-between">
        <button
          class="h-9 rounded-lg bg-white border border-gray-100 px-3 text-brand-secondary font-sans text-[12px] font-bold flex items-center justify-center gap-1.5 shadow-sm cursor-pointer border-none"
        ...
        <button class="w-9 h-9 rounded-lg bg-white border border-gray-100 flex items-center justify-center shadow-sm cursor-pointer border-none" @click="refreshActiveReview">
    ...
    <main class="max-w-md mx-auto px-margin-mobile pt-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
          <div class="flex items-start justify-between gap-3 mb-4">
            <div>
              ...
              <h2 class="font-serif text-[20px] font-bold text-brand-ink-strong mt-1">输入生辰数据</h2>
            </div>
          ...
          <div class="grid grid-cols-2 gap-2 mb-4">
            <button
              class="h-11 rounded-xl border-none font-sans text-[13px] font-bold transition-colors cursor-pointer"
              :class="gender === 'male' ? 'bg-brand-primary text-white' : 'bg-brand-paper text-brand-secondary'"
              @click="gender = 'male'"
            >
              男命
            </button>
            <button
              class="h-11 rounded-xl border-none font-sans text-[13px] font-bold transition-colors cursor-pointer"
              :class="gender === 'female' ? 'bg-brand-primary text-white' : 'bg-brand-paper text-brand-secondary'"
              @click="gender = 'female'"
            >
              女命
            </button>
          ...
          <div class="space-y-3">
            ...
            <label class="block">
              ...
              <input v-model="birthTime" type="time" class="w-full h-12 rounded-xl bg-brand-paper border-none px-3 font-sans text-[14px] text-brand-ink-strong outline-none focus:border-brand-primary" />
            </label>
            <label class="block">
              <span class="font-sans text-[12px] text-brand-secondary font-bold flex items-center gap-1.5 mb-1.5">
                <User :size="14" /> 命造姓名
              </span>
              <input v-model="profileName" maxlength="64" placeholder="可选" class="w-full h-12 rounded-xl bg-brand-paper border-none px-3 font-sans text-[14px] text-brand-ink-strong outline-none focus:border-brand-primary" />
            </label>
            <label class="block">
              <span class="font-sans text-[12px] text-brand-secondary font-bold flex items-center gap-1.5 mb-1.5">
                <MapPin :size="14" /> 出生地
              </span>
              <input v-model="birthPlace" maxlength="128" placeholder="可选，默认北京时间" class="w-full h-12 rounded-xl bg-brand-paper border-none px-3 font-sans text-[14px] text-brand-ink-strong outline-none focus:border-brand-primary" />
            </label>
          </div>

          <button
            class="mt-5 w-full h-12 rounded-xl bg-brand-primary text-white font-sans text-[14px] font-bold shadow-md disabled:opacity-60 disabled:shadow-none border-none cursor-pointer"
      ...
      <section v-else-if="viewState === 'error_state'" class="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-4">
        ...
        <div class="grid grid-cols-2 gap-2 mt-5">
          <button class="h-11 rounded-xl bg-brand-paper text-brand-ink-strong font-sans text-[13px] font-bold border-none cursor-pointer" @click="resetToInput">返回重填</button>
          <button
            class="h-11 rounded-xl bg-brand-primary text-white font-sans text-[13px] font-bold border-none cursor-pointer"
            @click="errorType.includes('insufficient') ? emit('navigate-to-tab', 'recharge') : handleOpenServiceContact()"
          >
            {{ errorType.includes('insufficient') ? '积分充值' : '联系客服' }}
          </button>
        </div>
      </section>
      ...
      <section v-else class="space-y-3">
        <div class="grid grid-cols-[1fr_auto] items-center gap-2">
          <div class="grid grid-cols-2 gap-1 rounded-lg bg-white border border-gray-100 p-1 shadow-sm w-full">
            <button
              class="h-9 rounded-lg font-sans text-[12px] font-bold transition-colors cursor-pointer border-none"
          ...
          <div class="px-3.5 py-1 rounded-lg bg-[#1E3A8A] text-white shadow-sm flex flex-col items-center justify-center leading-none">
            <span class="font-mono font-black text-[13px]">{{ reviewScore || '--' }}分</span>
            <span class="font-sans text-[8px] opacity-85 mt-0.5">格调</span>
          </div>
        </div>
        ...
        <div v-if="activeBranch === 'chart'" class="space-y-4">
          ...
          <div class="bg-white rounded-xl border border-[#D8E3F5] p-4 shadow-sm text-left">
            <div class="flex items-center gap-1.5 pb-2">
              <Sparkles :size="14" class="text-brand-primary" fill="currentColor" />
              <h3 class="font-serif text-[14px] font-bold text-brand-primary-strong">总评及起卦批注</h3>
            </div>
            <p class="font-serif text-[15px] font-bold text-brand-ink-strong mt-2 leading-snug">{{ summary?.title || '四柱总结生成中' }}</p>
            <p class="font-sans text-[12px] text-brand-secondary leading-relaxed mt-2">{{ summary?.risk || '风险推敲中。' }}</p>
            <p class="font-sans text-[12px] text-brand-ink leading-relaxed mt-2">{{ summary?.usage_guidance || '使用注意拟定中。' }}</p>
          </div>

          <div class="bg-white rounded-xl border border-[#D8E3F5] p-3 shadow-sm text-left">
            <div class="flex items-center justify-between mb-2">
              <h3 class="font-serif text-[14px] font-bold text-[#1D4ED8]">专项批写维度</h3>
              <span class="text-[11px] text-brand-secondary font-bold">解开需要扣减 {{ effectiveAspectUnlockPoints }} 积分/项</span>
            </div>
            <div class="grid grid-cols-3 gap-1.5">
              <button
                v-for="(aspect, index) in reviewAspects"
                :key="aspect.aspect_key"
                class="h-[46px] rounded-lg border px-1.5 text-center transition-colors flex flex-col items-center justify-center border-none cursor-pointer"
                :class="index === activeAspect ? 'bg-[#EAF1FF] text-[#1D4ED8] border-[#2563EB]' : 'bg-[#F8FAFF] text-brand-ink-strong'"
            ...
            <div v-if="selectedAspect" class="mt-2 rounded-lg bg-[#F8FAFF] p-3 text-left">
              <div class="flex items-start justify-between gap-3">
                <div>
                  <h4 class="font-serif text-[14.5px] font-bold text-brand-ink-strong">{{ selectedAspect.title }}</h4>
                </div>
                <button
                  v-if="!selectedAspect.is_unlocked"
                  class="h-9 px-3 rounded-lg bg-brand-primary text-white font-sans text-[12px] font-bold inline-flex items-center gap-1 disabled:opacity-60 border-none cursor-pointer"
                  :disabled="selectedAspectPending"
                  @click="void handleAspectClick(selectedAspect, activeAspect)"
                >
                  <UnlockKeyhole :size="14" />
                  {{ selectedAspectPending ? '生成中' : '全盘解锁' }}
                </button>
              </div>
              <p class="font-sans text-[13px] text-brand-ink leading-relaxed mt-2.5">
                {{ selectedAspect.content || (selectedAspectPending ? `正在调配运算，进度 ${unlockWaitingAttempt || 1} 次校验` : '点击上方解锁该维度后展示。') }}
              </p>
        ...
        <div v-else class="space-y-3">
          <!-- Table columns for Luck Cyces/Years -->
                ...
                <div class="overflow-x-auto no-scrollbar">
                  <div class="flex min-w-max">
                    <button
                      ...
                      :key="cycle.cycle_key"
                      class="w-[34px] min-h-[76px] py-1 px-0.5 transition-colors text-center flex flex-col items-center justify-center relative border-none"
                ...
                <div class="overflow-x-auto no-scrollbar">
                  <div class="flex min-w-max">
                    <button
                      ...
                      :key="item.year"
                      class="w-[38px] py-1 px-0.5 transition-colors flex flex-col items-center text-center relative border-none"
          ...
          <div v-if="selectedLuckCycle" class="bg-white rounded-xl border border-[#D8E3F5] p-3 shadow-sm text-left">
            <div class="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 class="font-serif text-[15px] font-bold text-[#1D4ED8] mt-1">大运十年主轴分析 · {{ selectedLuckCycle.display_ganzhi || selectedLuckCycle.ganzhi || '未起运' }}</h3>
                <p class="font-sans text-[12px] text-brand-secondary mt-1">{{ selectedLuckCycle.stem_ten_god || '过渡阶段' }} · 状态：{{ luckStatusText(selectedLuckCycle.render_status) }}</p>
              </div>
              <button
                class="h-9 px-3 rounded-lg bg-brand-primary text-white font-sans text-[12px] font-bold inline-flex items-center gap-1 disabled:opacity-60 border-none cursor-pointer"
              ...
              >
                ...
                <span>{{ selectedLuckCycle.render_status === 'completed' ? '重新生成' : '获取综评' }}</span>
              </button>
            </div>
            <div v-if="selectedLuckCycle.render?.result" class="mt-3 space-y-2 text-left">
              ...
              <p class="font-sans text-[12px] text-brand-ink leading-relaxed">{{ luckRenderText(selectedLuckCycle.render, 'core_theme') }}</p>
              <p class="font-sans text-[12px] text-emerald-700 leading-relaxed">{{ luckRenderText(selectedLuckCycle.render, 'opportunities') }}</p>
              <p class="font-sans text-[12px] text-red-650 leading-relaxed">{{ luckRenderText(selectedLuckCycle.render, 'risks') }}</p>
            ...
            <p v-else class="font-sans text-[13px] text-brand-secondary leading-relaxed mt-4">
              {{ selectedLuckCycle.render_status === 'processing' ? '正在推排大运主轴，由于命盘重合耗时稍长...' : '点击即可推演此大运阶段的核心契机与发展规划。' }}
            </p>
          </div>

          <!-- Selected Year Summary -->
          <div v-if="selectedLuckCycle && selectedLuckYearItem" class="bg-white rounded-xl border border-[#D8E3F5] p-3 shadow-sm text-left">
            <div class="rounded-lg bg-[#F8FAFF] p-3">
              <div class="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p class="font-sans text-[11px] text-brand-secondary mt-1">状态：{{ luckStatusText(selectedLuckYearItem.render_status) }}</p>
                </div>
                <button
                  class="h-9 px-3 rounded-lg bg-brand-primary text-white font-sans text-[12px] font-bold inline-flex items-center gap-1 disabled:opacity-60 border-none cursor-pointer"
              ...
              <div v-if="selectedLuckYearItem.render?.result" class="mt-3 space-y-2 text-left">
                ...
                <p class="font-sans text-[12px] text-red-650 leading-relaxed">{{ luckRenderText(selectedLuckYearItem.render, 'risks') }}</p>
                <p class="font-sans text-[12px] text-brand-secondary leading-relaxed">{{ luckRenderText(selectedLuckYearItem.render, 'action_guidance') }}</p>
              </div>
              <p v-else class="font-sans text-[13px] text-brand-secondary leading-relaxed mt-3">
                {{ selectedLuckYearItem.render_status === 'processing' ? '年度运程生成中...' : '点击即可推演此流年对您事业、财源、感情星的影响。' }}
              </p>
            </div>
          </div>
        </div>
